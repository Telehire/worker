...
export const PreviewInfoCard = (props: IProps) => {
  const {title, infoMap = [], handleEdit, isEmpty = false, emptyDesc = '', mulData} = props
  return (
    <Card sx={{borderRadius: 1, py: 6, px: 6,mb: 5}}>
      {
        !mulData && (
            ...
            if(v.name) {
              return <Grid key={index} item xs={12} sx={{display: 'flex',alignItems: 'center', py: 2, px: 4, mb: 2, borderBottom: '1px solid #F2F6FC'}}>
                ...
                <Grid item sx={{display: 'flex',alignItems: 'center', flex: 1}}>
                  ...
                  <Typography sx={{fontSize: 14, fontWeight: 400, color: '#3A354199'}}>{typeof v.desc === 'function'? v.desc() : v.desc}</Typography>
                </Grid>
                {
                  (v.click && !v.clickRender) && <Box component="img" src='/images/contract/pencil.png' sx={{cursor: 'pointer'}} height={14} onClick={() => {handleEdit(v.name)}} />
                }
                {
                  (v.click && v.clickRender) && v.clickRender()
                }
              </Grid>
            } else {
              return  <Typography key={index} sx={{fontSize: 14, fontWeight: 400, color: '#3A354199'}}>{typeof v.desc === 'function'? v.desc() : v.desc}</Typography>
            }
          })
        }
        {
                    ...
                    if(v.name) {
                      return <Grid key={index} item xs={12} sx={{display: 'flex',alignItems: 'center', py: 2, px: 4, mb: 2, borderBottom: '1px solid #F2F6FC'}}>
                        ...
                        <Grid item sx={{display: 'flex',alignItems: 'center', flex: 1, justifyContent: 'space-between'}}>
                          <Typography sx={{fontSize: 14, fontWeight: 500, color: '#303133', mr: 4}}>{v.value}</Typography>
                          {
                            v.click &&  <Box component="img" src='/images/contract/pencil.png' sx={{cursor: 'pointer'}} height={14} onClick={() => {handleEdit(v.name)}} />
                          }
                          {
                            !v.click &&  <Typography sx={{fontSize: 14, fontWeight: 400, color: '#3A354199'}}>{typeof v.desc === 'function'? v.desc() : v.desc}</Typography>
                          }
                        </Grid>
                      </Grid>
                    } else {
