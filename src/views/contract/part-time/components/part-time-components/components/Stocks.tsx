...
export const Stocks = (props: IProps) => {
  ...
  const checkInput = (list: any, date: any, total: any) => {
    let calcTotal = 0;
    let calcDate = 0;
    list.forEach((v: any) => {
    ...
    if(Number(total) !== Number(calcTotal) || Number(calcDate) !== Number(date)) {
      setOpenMes(true)
    } else {
      setStep(Step.Three)
    }
            ...
            <Controller
              ...
              render={({ field: { value, onChange } }) => (
                <Select label={t('Currency')} value={value} onChange={onChange} disabled>
                  <MenuItem value={10}>{t('人民币')}</MenuItem>
                </Select>
              )}
            />
  ...
  const StocksSecondPage = () => (
    ...
    >
      ...
      <DialogActions sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'end', alignItems: 'center' }}>
        ...
        <Button size='large' type='submit' variant='contained' onClick={handleSubmit(() => {checkInput(belongGetValues('belongList'), belongGetValues('scheduleOfBelong'), getValues('stocksNumbers')  )})}>
          {t('Next step')}
        </Button>
      </DialogActions>
    </Box>
