...
export const cusFormatDate = (date: any, type?: string) => {
  ...
  if(!type) {
    ...
    const last = Number(date) % (1000 * 60 * 60 * 24);
    const hour = Math.floor(last / (1000 * 60 * 60))
    const min = Math.floor((last - hour * 1000 * 60 * 60) / (1000 * 60))
    const second = Math.floor(last % (1000 * 60) / 1000);
    return `${year}年${month}月${day}日 ${hour > 9 ? hour : ('0' + hour)}:${min > 9 ? min : ('0' + min)}:${second > 9 ? second : ('0' + second)}`
